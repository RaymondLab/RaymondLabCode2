...
function [epx, epy, edge_thresh] = starburst_pupil_contour_detection2(I, cx, cy, edge_thresh, radiiPupil, min_features)
...
min_edge_thresh = 2;
N = 100;  % (Hannah = 100) Number of rays to use to detect feature points
min_candidate_features = N*min_features;  % Minimum number of pupil feature candidates
min_distance = radiiPupil(1)*.5;  % (Hannah = radiiPupil(1)*0.5) Distance from pupil center guess to start searching for edge
angle_spread = 60*pi/180;  % (Hannah = 60*pi/180)
min_change = 6;  % (Hannah = 6) Stop if sum of change in mean x and y over previous iteration is smaller than this (distance in pixels)
loop_count = 0;
max_loop_count = 10;
tcx(loop_count+1) = cx;
tcy(loop_count+1) = cy;
...
while edge_thresh > min_edge_thresh && loop_count <= max_loop_count
    ...
    if sqrt((tcx(loop_count+1)-cx).^2 + (tcy(loop_count+1)-cy).^2) < min_change  % (Hannah) 
    % if abs(tcx(loop_count+1)-cx) + abs(tcy(loop_count+1)-cy) < min_change  % Threshold change in pupil position over loops (Original)
        break;
    end
    cx = mean(epx);
    cy = mean(epy);
